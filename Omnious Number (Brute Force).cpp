...
/*
Input:
24 12943 3      (range: 24 to 12943 and Number of times these number should not occur: 3 or more)
3               (number of ominous digits = 3)
1 3 5           (ominous digits)

Output:
10997
*/
void solve(lli tc){
    int st, ed, k, n, result=0;
    cin >> st >> ed >> k >> n;
    int ominousDigits[n+1];
    aInput(ominousDigits, n);
    for(int i=st; i<=ed; i++){
        int d = i, cnt[11]={0};
        while(d){
            cnt[d%10]++;
