...
int n, m, a[15][15], dp[15][15];
bool vis[15][15];

int climb(int x, int y, int mx_lvl){
    if(a[x][y] == 3){
        return mx_lvl;
    }
    ...
    int ret = inf;
    vis[x][y] = true;
    if(y+1 < m and (a[x][y+1] == 1 or a[x][y+1] == 3) and !vis[x][y+1]){ //right
        ret = min(ret, climb(x, y+1, mx_lvl));
    }
    if(y-1 >= 0 and (a[x][y-1] == 1 or a[x][y-1] == 3) and !vis[x][y-1]){ //left
        ret = min(ret, climb(x, y-1, mx_lvl));
    }
    ...
    for(int i=x-1; i>=0; i--){ //up 
        if(a[i][y] == 1 or a[i][y] == 3){
            int lvl = abs(i-x);
            if(!vis[i][y]) ret = min(ret, climb(i, y, max(lvl, mx_lvl)));
            break;
        }
    }
