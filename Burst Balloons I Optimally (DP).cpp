...
int n;
vector<int> b;
lli dp[14][14000], vis[14];

lli score(int shot, int sum){
    if(shot==n) return 0;
    if(dp[shot][sum] != -1) return dp[shot][sum];
    lli ret=0;
    for(int i=0; i<n; i++){
        if(!vis[i]){
            vis[i] = 1;
            lli left, right, flag_left = 0, flag_right = 0, cost=0;
            for(int k=i+1; k<n; k++){
                if(!vis[k]){
                    right = k;
                    flag_right = 1;
            ...
            ret = max(ret, cost + score(shot+1, sum + cost));
            vis[i] = 0;
        }
    }
    return dp[shot][sum]=ret;
}
