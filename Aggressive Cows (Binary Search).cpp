...
bool getMinDist(int mn){
    int cnt=1, cur=v[0];
    for(int i=1; i<n; i++){
        if(v[i] - cur >= mn){ 
            //check if it is found that the difference between two stalls is greater than or equal 'mn'. That means mn is the minimum distance here.
            cnt++;
            cur = v[i];
        }
    }
...
void solve(lli tc){
    ...
    int first=0, last=1e9, mid, maxi = 0;
