...
int n, m, a[15][15], result_min_level;
bool vis[15][15];

void climb(int x, int y, int mx_lvl){
    if(a[x][y] == 3){
        result_min_level = min(result_min_level, mx_lvl);
        return;
    }
    if(vis[x][y]) return;

    vis[x][y] = true;
    if(y+1 < m and (a[x][y+1] == 1 or a[x][y+1] == 3)){ //right
        climb(x, y+1, mx_lvl);
    }
    if(y-1 >= 0 and (a[x][y-1] == 1 or a[x][y-1] == 3)){ //left
        climb(x, y-1, mx_lvl);
    }

    for(int i=x-1; i>=0; i--){ //up 
        if(a[i][y] == 1 or a[i][y] == 3){
            int lvl = abs(i-x);
            climb(i, y, max(lvl, mx_lvl));
            break;
        }
    }
...
void solve(lli tc){
    ...
    input2D(a, n, m);
    mem(vis, 0);
    result_min_level = inf;
    climb(n-1, 0, 0);
    print(result_min_level);
}
