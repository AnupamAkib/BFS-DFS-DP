...
//https://practice.geeksforgeeks.org/problems/steps-by-knight5927/1?page=9&company[]=Amazon&sortBy=submissions
...
int minimumMoves(pair<int, int> head){
    ...
    while(!Q.empty()){
        head = Q.front();
        Q.pop();
        if(head.first == target.first and head.second == target.second){
            return dis[head.first][head.second];
        }
        for(int i=0; i<8; i++){
            int x = head.first + kx[i];
            int y = head.second + ky[i];
